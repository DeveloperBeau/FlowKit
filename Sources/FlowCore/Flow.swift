/// A cold asynchronous stream of non-failing values. Cold by construction —
/// the body closure runs each time a collector is attached, and each collector
...
///
/// The two types are distinct — no shared protocol, no type erasure — so
...
/// executed when the flow is created — only when `collect(_:)` is called.
